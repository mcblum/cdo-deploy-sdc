...
make_sdc_user() {
  sdc_home_default=/home/sdc
  read -p "Home dir for the sdc user [$sdc_home_default]:" sdc_home
  sdc_home=${sdc_home:-$sdc_home_default}
  mkdir -p "$sdc_home"

  echo "Creating user: sdc"
  sudo adduser --gecos "" --disabled-password sdc --home "$sdc_home"

  # Set permissions for sdc user's home dir
  sudo chown sdc:sdc "$sdc_home"
}

# Create sdc user if it does not exist and set home directory
configure_users_and_groups() {
  if id sdc >/dev/null 2>&1; then
      sdc_home=$( getent passwd "sdc" | cut -d: -f6 )
      echo "Found existing user: sdc with a home dir location of $sdc_home"
  else
      make_sdc_user
  fi
...
# Decode SDC bootstrap data and extract the needed pieces
bootstrap() {
